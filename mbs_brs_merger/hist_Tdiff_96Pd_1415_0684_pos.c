...
void hist_Tdiff_96Pd_1415_0684_pos()
{
	TString str_ri = "96Pd";
	Double_t Ed = 1415;
	Double_t Ef = 684;
	...
	TH1I *h1_Tdiff = new TH1I(
			...
			Form("T(%.0f keV)-T(%.0f keV) {%s}; Tdiff [ps]",  Ed, Ef, str_ri.Data()),
			 400,- 2e3, 2e3);
	TH1I *h1_Tdiff_L = new TH1I(
			Form("h1_Tdiff_L_%s_%.0f_%.0f",  str_ri.Data(), Ed, Ef),
			Form("T(%.0f keV)-T(%.0f keV) {%s}; Tdiff [ps]",  Ed, Ef, str_ri.Data()),
			 400,- 2e3, 2e3);
	TH1I *h1_Tdiff_R = new TH1I(
			Form("h1_Tdiff_R_%s_%.0f_%.0f",  str_ri.Data(), Ed, Ef),
			Form("T(%.0f keV)-T(%.0f keV) {%s}; Tdiff [ps]",  Ed, Ef, str_ri.Data()),
			 400,- 2e3, 2e3);
	TH1I *h1_Tdiff_gft = new TH1I(
			Form("h1_Tdiff_gft_%s_%.0f_%.0f",  str_ri.Data(), Ed, Ef),
			Form("T(%.0f keV)-T(%.0f keV) {%s}; Tdiff [ps]",  Ed, Ef, str_ri.Data()),
			 400,- 2e3, 2e3);
	...
	for (ient=0; ient<nent; ient++, loop_couneter++)
	{
		...
		for (ihit=0; ihit<nhit; ihit++)
		{
			...
			for(jhit=0; jhit<nhit; jhit++) if(ihit!=jhit)
			{
				...
				h1_Tdiff_gft->Fill( gft.flight_time(iKL, pmerg->ppac_x, pmerg->ppac_y,0) - gft.flight_time(jKL, pmerg->ppac_x, pmerg->ppac_y,0));
				if (pmerg->ppac_x>0) 
					h1_Tdiff_L->Fill(iftle-jftle);
				if (pmerg->ppac_x<0) 
					h1_Tdiff_R->Fill(iftle-jftle);
	...
	h1_Tdiff->Draw();
	h1_Tdiff_L->SetLineColor(kRed);
	h1_Tdiff_L->Draw("same");
	h1_Tdiff_R->SetLineColor(kGreen);
	h1_Tdiff_R->Draw("same");
