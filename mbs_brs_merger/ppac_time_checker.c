...
void ppac_time_checker()
{
	...
	TH2I *h2_ppch_ftle = new TH2I("h2_ppch_ftle","h2_ppch_ftle",
			16,0,16,
			1000, -2000e3, -1500e3
			//2*COARSE_CT_RANGE, -COARSE_CT_RANGE*CYCLE_TIME, COARSE_CT_RANGE*CYCLE_TIME
			);
	TH2I *h2_ppch_dftle = new TH2I("h2_ppch_dftle","h2_ppch_dftle",
			16,0,16,
			2000, -1000e3, 1000e3
			);

	TH2I *h2_ppch_stot = new TH2I("h2_ppch_stot","h2_ppch_stot",
			16,0,16,
			COARSE_CT_RANGE, 0, COARSE_CT_RANGE*CYCLE_TIME/8
			);
	TH2I *h2_ppac_position = new TH2I("h2_ppac_position","h2_ppac_position",
		600,- 60, 60,
		600,- 60, 60
		//1000,-100e3,100e3,
		//1000,-100e3,100e3
	...
	TFile *fileout = new TFile(Form("./temp.root"),"recreate");
	...
	for (
			...
			ient_mbs<nent_mbs/100 && ient_brs<nent_brs;
...
/////////// Do what you want below here ///////////
			...
			for (int idx=0; idx<size; idx++)
			{
				...
				if (itam== 6)
				{
					h2_ppch_stot->Fill(icha, istot);
					h2_ppch_ftle->Fill(icha, iftle);
				}
				if (itam== 6 && icha== 0)
				{
					for (int jdx=0; jdx<size; jdx++) if (pmbs->OutputEvent_hit_TAM[jdx]== 6) 
					{
						h2_ppch_dftle->Fill( pmbs->OutputEvent_hit_PCHA[jdx], pmbs->OutputEvent_hit_FTle[jdx]-iftle );
					}
				}
				...
				if (itam== 6 && icha== 0)
				{
					for (int jdx=0; jdx<size; jdx++) if (pmbs->OutputEvent_hit_TAM[jdx]== 6) if (abs(pmbs->OutputEvent_hit_FTle[jdx]-iftle)<200e3)
					{
...
////////////////////// do what you want over here ///////////////
	...
	pad = c1->cd(); 
	h2_ppac_position->Draw("colz");

	TCanvas *c2 = new TCanvas("c2","c2"); 
	pad = c2->cd(); 
	h2_ppch_ftle->Draw("colz");
